...
이 작업에서 사용하는 `Get-WindowsCapability -Online` 명령은 **관리자 권한이 필요**하다.

본 가이드에서는 VS Code 전체를 관리자 권한으로 실행하기보다, **VS Code는 일반 권한으로 유지하고 필요한 명령만 Windows의 `sudo` 기능을 이용하여 관리자 권한으로 실행하는 방식**을 권장한다.

```mermaid
flowchart LR
    A[일반 권한 VS Code] --> B[Integrated Terminal]
    B --> C[sudo]
    C --> D[UAC 승인]
    D --> E[Get-WindowsCapability]
    E --> F[VS Code Terminal에서 결과 확인]
```

!!! note "관리자 권한이 필요한 이유"
    다음 명령을 일반 권한 PowerShell에서 바로 실행하면 권한 상승이 필요하다는 오류가 발생할 수 있다.
    ...
    이는 명령 자체의 오류가 아니라 **현재 PowerShell이 관리자 권한으로 실행되지 않았기 때문**이다.
...
도움말이 표시되면 `sudo.exe` 자체는 사용할 수 있는 Windows 환경이다.

!!! note "Sudo for Windows 지원 버전"
    Microsoft 기준으로 Sudo for Windows는 **Windows 11 24H2 이상**에서 제공된다.

    Windows 버전이 낮다면 Windows Update 상태를 확인하거나, 이 문서의 [6.7 Sudo를 사용할 수 없는 경우](#67-sudo를-사용할-수-없는-경우)를 참고한다.
...
이 경우 Windows에 `sudo.exe`는 존재하지만 기능이 비활성화된 상태다.

Windows 설정에서 다음 메뉴로 이동한다.
...
`Enable sudo`를 **On**으로 변경한다.

!!! warning "Sudo 보안 주의사항"
    `sudo`는 필요한 명령을 관리자 권한으로 실행하는 기능이다.

    특히 현재 콘솔에 관리자 프로세스를 연결하는 실행 모드는 편리하지만 보안상 고려할 점이 있으므로, 신뢰할 수 있는 명령에만 사용한다.
...
| 모드 | 설정값 | 동작 |
|---|---|---|
| 새 창 | `forceNewWindow` | 관리자 명령을 별도의 콘솔 창에서 실행 |
| 입력 차단 | `disableInput` | 현재 콘솔에서 실행하지만 관리자 프로세스 입력 차단 |
| 현재 창 | `normal` | 현재 콘솔에서 관리자 명령을 실행하고 입출력 사용 |

기본 설정은 일반적으로 **새 창(`forceNewWindow`)** 방식이다.

이 상태에서 다음과 같은 명령을 실행하면:

```powershell
sudo powershell.exe ...
```

별도의 관리자 PowerShell 창이 열리고, 명령 실행이 끝나는 즉시 창이 닫힐 수 있다.

따라서 **VS Code Terminal 안에서 실행 결과를 계속 확인하려는 경우** `normal` 모드를 사용할 수 있다.
...
형태로 현재 창에서 실행되는 모드를 선택한다.

또는 관리자 권한 콘솔에서 다음 명령으로 설정할 수 있다.

```powershell
sudo config --enable normal
```

!!! warning "`normal` 모드 사용 시"
    Microsoft는 `normal` 모드가 현재 콘솔과 관리자 프로세스의 입출력을 연결하기 때문에 편리하지만 추가적인 보안 고려가 필요하다고 안내한다.

    학습용 PC에서 VS Code Terminal의 실행 결과를 바로 확인하려는 경우에는 편리하지만, 업무 PC 또는 보안 정책이 적용된 환경에서는 조직 정책을 우선한다.

!!! tip "새 창이 열렸다가 바로 닫히는 경우"
    `sudo` 명령은 실행되지만 관리자 창이 잠깐 표시된 후 바로 사라진다면 오류가 아니라 **Sudo가 `forceNewWindow` 모드로 설정되어 있기 때문일 가능성이 높다.**

    VS Code Terminal에서 결과를 직접 확인하려면 `normal` 모드를 사용한다.
...
실행하면 Windows UAC(User Account Control) 확인 창이 표시된다.

관리자 권한 실행을 승인한다.

정상적인 경우 VS Code Terminal에 다음과 같은 결과가 표시된다.
...
`State`가 다음과 같다면 VBScript 기능이 설치된 상태다.

```text
Installed
```

!!! success "VBScript 설치 상태 확인 완료"
    다음과 같이 표시되면 VBScript Feature on Demand가 정상적으로 설치되어 있다.
...
```text
현재 VS Code PowerShell
        ↓
sudo
        ↓
관리자 권한 powershell.exe
        ↓
-Command "..."
        ↓
Get-WindowsCapability 실행
```
...
바깥쪽 PowerShell에서 `$_`를 먼저 해석하지 않고 **관리자 권한으로 새로 실행되는 PowerShell에 그대로 전달하기 위해 `$` 앞에 백틱(``)을 사용**한다.

또한 패턴 문자열은 다음처럼 작은따옴표를 사용한다.
...
이 경우 명령을 다음 형태 그대로 사용한다.

```powershell
sudo powershell.exe -NoProfile -Command "Get-WindowsCapability -Online | Where-Object { `$_.Name -like 'VBSCRIPT*' }"
```

!!! tip "명령을 그대로 사용하는 것을 권장"
    이 명령은 PowerShell → `sudo` → PowerShell 구조이기 때문에 따옴표와 `$` 처리에 주의해야 한다.

    처음 환경을 구성할 때는 명령을 임의로 수정하기보다 위 형태 그대로 실행하는 것이 좋다.
...
!!! tip "설치 방법 통일"
    본 가이드에서는 VBScript 설치가 필요한 경우 **Windows의 선택적 기능 화면을 이용하는 방식**을 기준으로 한다.

    설치 후에는 다시 다음 명령을 실행하여 `State : Installed`가 표시되는지 확인한다.
...
!!! note "메뉴 위치"
    Windows 11 빌드에 따라 선택적 기능 메뉴의 세부 경로가 달라질 수 있다.

    메뉴 위치를 직접 찾기보다 Windows 설정 검색에서 **선택적 기능**을 검색하는 방식을 권장한다.
...
VS Code의 Integrated Terminal에서 다음 명령을 실행한다.
...
!!! tip "권장 방식"
    평상시 VBScript 작성과 실행에는 관리자 권한이 필요하지 않다.

    따라서 **VS Code는 일반 권한으로 사용하고, Windows Capability 확인이나 시스템 변경처럼 실제로 관리자 권한이 필요한 명령만 `sudo` 또는 관리자 PowerShell로 실행**하는 방식을 권장한다.
...
이 결과가 확인되었다면 Windows 11의 VBScript 실행 기능이 준비된 상태다.

다음 단계에서는 실제 `.vbs` 파일을 작성하고 `cscript.exe`로 실행하여 **VBScript 엔진 자체가 정상적으로 동작하는지 검증**한다.
...
!!! note "`Ctrl + Shift + B`와 기본 Build Task"
    ...
    `kind: "build"`는 해당 Task를 VS Code의 **Build Task 그룹**에 등록한다는 의미이고,  
    `isDefault: true`는 여러 Build Task 중 이 Task를 **기본 Build Task**로 지정한다는 의미이다.
    ...
    즉, 이 프로젝트에서 `Ctrl + Shift + B`를 누른다고 해서 실제로 VBScript를 **빌드(Build)** 하는 것은 아니다.

    VS Code가 제공하는 **기본 Build Task 단축키를 VBScript 실행 Task에 연결하여 사용하는 것**이다.

    Java나 Spring Boot 프로젝트에서는 `Ctrl + Shift + B`가 실제 컴파일이나 패키징 등의 Build Task에 연결될 수 있으므로,  
    `Ctrl + Shift + B`를 모든 언어의 공통적인 **프로그램 실행 단축키**로 이해하면 안 된다.
...
VS Code를 일반 권한으로 계속 사용하려면 다음 명령처럼 **해당 명령만 `sudo`로 관리자 권한 실행**한다.
...
!!! note "상세 설명"
    자세한 절차는 [3단계 - VBScript Feature on Demand 상태 확인](#6-3단계-vbscript-feature-on-demand-상태-확인)을 참고한다.
...
VS Code Integrated Terminal에서는 다음 명령으로 확인한다.
...
`Installed`가 아니라면 Windows 설정에서 다음 항목을 검색한다.
...
목록에서 `VBScript`를 검색하여 설치한 후 다시 상태를 확인한다.

!!! tip "권장 확인 방법"
    본 가이드에서는 VBScript 상태 확인과 설치 절차를 `Get-WindowsCapability`와 Windows 선택적 기능 화면으로 통일한다.

    별도의 Windows 이미지 관리 명령은 사용하지 않는다.
...
- [ ] `Get-WindowsCapability -Online` 실행에는 관리자 권한이 필요함을 확인했다.
- [ ] 필요 시 VS Code Terminal에서 `sudo`로 관리자 권한 명령을 실행할 수 있다.
- [ ] `hello.vbs` 파일을 생성했다.
